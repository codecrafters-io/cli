...
// CustomShallowStorage where the shallow commits are stored, an internal to
// manipulate the shallow file
type CustomShallowStorage struct {
	dir *dotgit.DotGit
}
...
// newline.
func (s *CustomShallowStorage) SetShallow(commits []plumbing.Hash) error {
...
func (s *CustomShallowStorage) Shallow() ([]plumbing.Hash, error) {
	f, err := s.dir.Shallow()
	if f == nil || err != nil {
		return nil, err
	}
